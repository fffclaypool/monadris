...
class GridSpec extends AnyFlatSpec with Matchers:
  ...
  it should "detect completed rows" in {
    ...
    filledGrid.completedRows shouldBe List(19)
  }
  ...
  it should "detect multiple completed rows" in {
    ...
    // 2行埋める
    val filledGrid = (0 until 10).foldLeft(grid) { (g, x) =>
      g.place(Position(x, 18), filled).place(Position(x, 19), filled)
    }

    filledGrid.completedRows should contain allOf (18, 19)
  }
  ...
  it should "clear rows and add empty rows at top" in {
    ...
    val clearedGrid = filledGrid.clearRows(List(19))

    // 最下行は空になるはず
    clearedGrid.isEmpty(Position(0, 19)) shouldBe true
    // 最上行も空
